<script>
	...
	for (const ipaObj of ipaObjects) {
		...
		for (const letter of letters) {
			...
			let sumLetter = Object.entries(ipaColors).find(
				([key, value]) => value === letterColor
			)?.[0];
			...
			if (letterColor) {
				if (!bubbleData.find((d) => d.color === letterColor)) {
					...
					if (letterColor.includes("pink") && letterColor.includes("yellow")) {
						...
						letterColor.includes("green") &&
						letterColor.includes("yellow")
					) {
					...
					if (letter === "͡ʃ" || lettersCounted.includes(sumLetter)) {
						continue;
					}
	...
	onMount(() => {
		...
		simulation = forceSimulation(bubbleData)
			...
			.force(
				"collision",
				forceCollide((d) => (d.count * 9 + 12) * scaleFactor)
			)
			.on("tick", ticked);
		...
		const node = graphGroup
			...
			.attr("cursor", "pointer")
			.on("click", (event, d) => {
				playIPAAudio(`audio/ipa-${d.sumLetter}.mp3`);
			})
			.on("mouseenter", (event, d) => {
				setHoverColor(d.color);
			})
		...
		// volume icon
		fetch(`${base}/assets/volume-icon.svg`)
			...
			.then((svgContent) => {
				node
					...
					.attr(
						"transform",
						(d) =>
							`translate(0, ${1.75 * d.count * getScaleFactor(width) + 20})`
					)
					.html(svgContent)
					.each(function () {
